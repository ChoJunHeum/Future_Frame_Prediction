...
class RetroNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, 128, kernel_size=7, stride=2, padding=1, bias=False)
        )
        ...
        self.conv5 = nn.Sequential(
            nn.ConvTranspose2d(128, 256, kernel_size=3, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True)
        )

        self.outconv = nn.Sequential(
            nn.Conv2d(256, 3, kernel_size=7, stride=1, padding=1, bias=False)
        )
