    ...
    while training:
        for indice, clips, flow_strs in train_dataloader:
            ...
            if len(new_areas)!=0:
                ...
                Fgt_flow_input_1 = torch.cat([frame_1.unsqueeze(2), f_target.unsqueeze(2)], 2)
                Fpred_flow_input_1 = torch.cat([frame_1.unsqueeze(2), FG_frame.unsqueeze(2)], 2)

                F_flow_gt_1 = (flow_net(Fgt_flow_input_1 * 255.) / 255.).detach()  # Input for flownet2sd is in (0, 255).
                F_flow_pred_1 = (flow_net(Fpred_flow_input_1 * 255.) / 255.).detach()

                Fgt_flow_input_2 = torch.cat([frame_2.unsqueeze(2), f_target.unsqueeze(2)], 2)
                Fpred_flow_input_2 = torch.cat([frame_2.unsqueeze(2), FG_frame.unsqueeze(2)], 2)

                F_flow_gt_2 = (flow_net(Fgt_flow_input_2 * 255.) / 255.).detach()  # Input for flownet2sd is in (0, 255).
                F_flow_pred_2 = (flow_net(Fpred_flow_input_2 * 255.) / 255.).detach()

                Fgt_flow_input_3 = torch.cat([frame_3.unsqueeze(2), f_target.unsqueeze(2)], 2)
                Fpred_flow_input_3 = torch.cat([frame_3.unsqueeze(2), FG_frame.unsqueeze(2)], 2)

                F_flow_gt_3 = (flow_net(Fgt_flow_input_3 * 255.) / 255.).detach()  # Input for flownet2sd is in (0, 255).
                F_flow_pred_3 = (flow_net(Fpred_flow_input_3 * 255.) / 255.).detach()

                Fgt_flow_input_4 = torch.cat([frame_4.unsqueeze(2), f_target.unsqueeze(2)], 2)
                Fpred_flow_input_4 = torch.cat([frame_4.unsqueeze(2), FG_frame.unsqueeze(2)], 2)

                F_flow_gt_4 = (flow_net(Fgt_flow_input_4 * 255.) / 255.).detach()  # Input for flownet2sd is in (0, 255).
                F_flow_pred_4 = (flow_net(Fpred_flow_input_4 * 255.) / 255.).detach()
                ...
                Bgt_flow_input_1 = torch.cat([FG_frame.unsqueeze(2), f_target.unsqueeze(2)], 2)
                Bpred_flow_input_1 = torch.cat([FG_frame.unsqueeze(2), BG_frame.unsqueeze(2)], 2)

                B_flow_gt_1 = (flow_net(Bgt_flow_input_1 * 255.) / 255.).detach()  # Input for flownet2sd is in (0, 255).
                B_flow_pred_1 = (flow_net(Bpred_flow_input_1 * 255.) / 255.).detach()

                Bgt_flow_input_2 = torch.cat([frame_4.unsqueeze(2), f_target.unsqueeze(2)], 2)
                Bpred_flow_input_2 = torch.cat([frame_4.unsqueeze(2), BG_frame.unsqueeze(2)], 2)

                B_flow_gt_2 = (flow_net(Bgt_flow_input_2 * 255.) / 255.).detach()  # Input for flownet2sd is in (0, 255).
                B_flow_pred_2 = (flow_net(Fpred_flow_input_2 * 255.) / 255.).detach()

                Bgt_flow_input_3 = torch.cat([frame_3.unsqueeze(2), f_target.unsqueeze(2)], 2)
                Bpred_flow_input_3 = torch.cat([frame_3.unsqueeze(2), BG_frame.unsqueeze(2)], 2)

                B_flow_gt_3 = (flow_net(Bgt_flow_input_3 * 255.) / 255.).detach()  # Input for flownet2sd is in (0, 255).
                B_flow_pred_3 = (flow_net(Bpred_flow_input_3 * 255.) / 255.).detach()

                Bgt_flow_input_4 = torch.cat([frame_2.unsqueeze(2), f_target.unsqueeze(2)], 2)
                Bpred_flow_input_4 = torch.cat([frame_2.unsqueeze(2), BG_frame.unsqueeze(2)], 2)

                B_flow_gt_4 = (flow_net(Bgt_flow_input_4 * 255.) / 255.).detach()  # Input for flownet2sd is in (0, 255).
                B_flow_pred_4 = (flow_net(Bpred_flow_input_4 * 255.) / 255.).detach()
