    ...
    while training:
        for indice, clips, flow_strs in train_dataloader:
            ...
            if len(new_areas)!=0:
                ...
                F_flow_gt = (flow_net(Fgt_flow_input * 255.) / 255.).detach()  # Input for flownet2sd is in (0, 255).
                ...
                Bgt_flow_input = torch.cat([frame_2.unsqueeze(2), b_target.unsqueeze(2)], 2)
                Bpred_flow_input = torch.cat([frame_2.unsqueeze(2), BG_frame.unsqueeze(2)], 2)

                B_flow_gt = (flow_net(Bgt_flow_input * 255.) / 255.).detach()  # Input for flownet2sd is in (0, 255).
