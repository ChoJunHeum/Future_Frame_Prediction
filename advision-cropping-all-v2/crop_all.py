...
import crop_utils
from crop_utils import VideoDataset


parser = argparse.ArgumentParser()
parser.add_argument('--batch_size', '-bs', default=16, type=int)
parser.add_argument('--confidence', '-c', default=.6, type=float, required=True)
...
if __name__ == "__main__":
    ...
    '''
    outpath = 'datasets/avenue_cropped/'
    dpath = 'datasets/avenue/training/'
    ...
    for num_folder in dirs:
        ...
        for batches in tqdm(vdatasets.get_batches(batch_size=batch_size), total=total):
            ...
            batch_areas = crop_utils.batch_detect(model, batch_frames_d, device=device)
            ...
            for i in range(len(batch_frames_d)):
                ...
                crop_utils.crop_save(frames, new_areas, outdirs, batch_num_detect[i])
