...
def batch_detect(model, frame_d, device):
...
def crop_save(frames, areas, outdirs, num_detect):
    '''
    ...
    for num_area, area in enumerate(areas):
        ...
        for i, frame in enumerate(frames):
            ...
            # one_area.append(frame.crop(area))
        # results.append(one_area)
    return None


def get_resized_area(areas, max_w, max_h, confidence, factor_x=1.5, factor_y=1.2):
    '''
    ...
    new_areas = []
    for area in areas:
        if area[-1] == 0 and area[4] > confidence:
